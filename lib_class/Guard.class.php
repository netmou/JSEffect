 ...
 class Guard extends Exception
 {
     ...
     public static function formatTrace($e)
     {
         ...
         foreach ($traces as $k => $trace) {
             ...
             if (isset($trace['args'])) {
                 $traceLog .= ' With the args in json format: '.json_encode($trace['args']);
             }
             $traceLog .= "\n";
         }
         return $traceLog;
     ...
     {
         $desc = "Your program has a problem, you can modify the error_report setting to avoid displaying debug-info! \n";
