...
class Lunar {
    ...
    /**
     * 将阳历转换为阴历，仅提供数字格式的年月日
     * @param year 公历-年
     * @param month 公历-月
     * @param date 公历-日
     */
    function getLunarInfo($year, $month, $date) {
        $yearData = $this->lunarInfo[$year - $this->MIN_YEAR];
        return $this->getLunarInfoByBetween($year, $this->getDaysBetweenSolar($year, $month, $date, $yearData[1], $yearData[2]));
    }
    ...
    /**
     * 根据距离正月初一的天数计算阴历日期，返回数字格式信息
     * @param year 阳历年
     * @param between 天数
     * @return array(year,month,date,leap),其中month<13,用leap为1标识闰月月份中的后月
     */
    function getLunarInfoByBetween($year, $between) {
        if ($between == 0) {
            return array('year' => $year, 'month' => 1, 'date' => 1, 'leap' => 0);
        } else {
            ...
            $m = ($leapMonth != 0 && $t == $leapMonth + 1) ? $t - 1 : ($leapMonth != 0 && $leapMonth + 1 < $t ? ($t - 1) : $t);
            return array('year' => $year, 'month' => $m, 'date' => $e, 'leap' => ($leapMonth != 0 && $t == $leapMonth + 1) ? 1 : 0);
        }
    }

    /**
     * 根据距离正月初一的天数计算阴历日期，返回汉字格式信息
     * @param year 阳历年
     * @param between 天数
     */
